// Local include(s):
// $Id: DataSkim.cxx 141731 2013-03-25 09:47:24Z mliu $
...
// if (m_vxp_n < 1 || m_vxp_nTracks->at(0)< 3) throw SError(SError::SkipEvent);
  ...
  if(m_el_pt->at(i)>20000&&m_el_tightPP->at(i)&&(m_el_author->at(i)==1||m_el_author->at(i)==3)&&eta<2.7) tight_el++;
  if(m_el_pt->at(i)>20000&&m_el_mediumPP->at(i)&&(m_el_author->at(i)==1||m_el_author->at(i)==3)&&eta<2.7) 
  {
  medium_el++;
  TLorentzVector tmp;
  ...
  if(m_mu_staco_pt->at(i)>10000&&m_mu_staco_tight->at(i)&&eta<2.7) tight_muon++;
  //if(m_mu_staco_pt->at(i)>7000&&m_mu_staco_medium->at(i)&&eta<2.7) 
  if(m_mu_staco_pt->at(i)>10000&&eta<2.7) 
  {
  medium_muon++;
  TLorentzVector tmp;
...
// if(Mll<5000) throw SError( SError::SkipEvent ); 
} 
  ...
  if(!isMC&&m_prop_filter=="wgg")
{
  ...
  if(m_prop_filter=="zg"||m_prop_filter=="wenugskim")
{
